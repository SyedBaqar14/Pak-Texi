...
public class ActivityEmail extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        nextICON.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (phonenumber.getText().toString().equals("") || phonenumber.getText().toString().equalsIgnoreCase(getString(R.string.sample_phone_number))) {

                    displayMessage(getString(R.string.phone_validation));

                } else {

                    if ((!isValiPhone(phonenumber.getText().toString()))) {

                        displayMessage(getString(R.string.phone_validation));

                    } else {
                        Utilities.hideKeyboard(ActivityEmail.this);
                        SharedHelper.putKey(ActivityEmail.this, "mobile", phonenumber.getText().toString());
    ...
    private boolean isValiPhone(String email) {
        String EMAIL_PATTERN = "^([0-9\\\\+]|\\\\(\\\\d{1,3}\\\\))[0-9\\\\-\\\\. ]{3,15}$";
        Pattern pattern = Pattern.compile(EMAIL_PATTERN);
        Matcher matcher = pattern.matcher(email);
        return matcher.matches();
    }
